...
async def make_api_request(method: str, endpoint: str, token: Optional[str] = None, json: Optional[Dict] = None, data: Optional[Dict] = None, files: Optional[Dict] = None):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.request(method, url, headers=headers, json=json, data=data, files=files, timeout=10) as response:
...
async def register_api_call(username: str, password: str, gemini_api_key: str) -> APIResponse:
    ...
    try:
        response = await make_api_request('POST', 'register', json=data)
